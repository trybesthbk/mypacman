...
void keyOperations(){
	...
	if (keyStates['a']){
		...
		if (!bitmap.at(x1Quadrant).at((int)(y-12)/squareSize)&&\
			!bitmap.at(x1Quadrant).at((int)(y+12)/squareSize)){
			xIncrement -= myspeed / squareSize;
			rotation = 2;
		}
	}
	if (keyStates['d']){
		...
		if (!bitmap.at(x2Quadrant).at((int)(y-12) / squareSize)&&\
			!bitmap.at(x2Quadrant).at((int)(y+12) / squareSize)){
			xIncrement += myspeed / squareSize;
			rotation = 0;
		}
	}
	if (keyStates['w']){
		...
		if (!bitmap.at((int)(x-12)/squareSize).at(y1Quadrant)&&
			!bitmap.at((int)(x+12)/squareSize).at(y1Quadrant)){
			yIncrement -= myspeed / squareSize;
			rotation = 3;
		}
	}
	if (keyStates['s']){
		...
		if (!bitmap.at((int)(x-12) / squareSize).at(y2Quadrant)&&\
		!bitmap.at((int)(x+12) / squareSize).at(y2Quadrant)){
			yIncrement += myspeed / squareSize;
			rotation = 1;
		}
	}
